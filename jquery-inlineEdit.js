...
$.fn.editInPlace.defaults = {
	...
	hover_class:		"hover",  // string: class added to root element during hover. Will override bg_over and bg_out
...
$.extend(InlineEditor.prototype, {
	...
	replaceContentWithEditor: function() {
        ...
        if( this.settings.field_type === "select" ){
            editorElement.selectBox();
        }
	},
	...
	createSelectEditor: function() {
		...
		if( _.isFunction( this.settings.select_options ) )
		{
		    editor = this.settings.select_options( this.originalValue, this.dom );
		    editor.attr("name", "inplace_value")
	              .addClass("inplace_field");
		}
	    else if (_.isObject(this.settings.select_options) && !_.isArray(this.settings.select_options))
	    {
	        editor = this.settings.select_options.clone();
	        editor.attr("name", "inplace_value")
	              .addClass("inplace_field");
        }
	    else
	...
	bindSubmitOnEnterInInput: function() {
        ...
        // Disabled because it doesn't play well with keyboard navigation on the few dialog.
/*		var that = this;
	...
	triggerCallback: function(aCallback /*, arguments */) {
		...
		var callbackArguments = Array.prototype.splice.call(arguments, 1, arguments.length-1);
